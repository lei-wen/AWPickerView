...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        self.pickerMode = AWPickerCityMode;
    }
    return self;
}

#pragma mark - set方法
- (void)setPickerMode:(AWPickerMode)pickerMode {
    _pickerMode = pickerMode;
    if (_pickerMode == AWPickerCityMode) {
        [self loadCityInfoData];
        [self addSubview:self.cityPickerView];
    } else if (_pickerMode == AWPickerDateMode) {
        if (_cityPickerView) {
...
- (NSDictionary *)setCityName:(NSDictionary *)dic {
    NSDictionary *tempDic = @{@"areaId": dic[@"areaId"], @"areaName": dic[@"areaName"]};
    return tempDic;
}
